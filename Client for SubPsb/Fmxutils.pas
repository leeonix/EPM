{ 文件管理,from DELPHI DEMOS, and modified something  denoted by csb}
unit FmxUtils;

interface

uses SysUtils, Windows, Classes, Consts,FileCtrl;
...
begin
   ...
   if not DirectoryExists(directory) then   //如果路径不存在则建立目标路径；2000/12/26 tuzz
...
GetMem(CopyBuffer, ChunkSize); { allocate the buffer }
  try
    ...
    if Source < 0 then raise EFOpenError.CreateFmt('', [FileName]);
    try
      Dest := FileCreate(Destination); { create output file; overwrite existing }
      if Dest < 0 then raise EFCreateError.CreateFmt('', [Destination]);
      try
        repeat
...
function ExecuteFile(const FileName, Params, DefaultDir: string;
  ShowCmd: Integer): THandle;
var
  zFileName, zParams, zDir: array[0..510] of Char;
begin
